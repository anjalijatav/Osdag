...
def flange_force( col_d, col_f_t, axial_force, moment_load, area_f, total_area):
    """
    Args:
       col_d: Overall depth of the column section in mm (float)
       col_f_t: Thickness of flange in mm (float)
    ...
    """
    tm = moment_load
    axial_force_f = area_f * axial_force/total_area                         #kN
    return round((tm*1000)/(col_d - col_f_t), 2) + (axial_force_f)        # kN


# Force in flange (kN)  [Reference:  N. Subramanian (page 428)]
def web_force(col_d, col_f_t, col_w_t, axial_force, total_area):
    """
    Args:
       col_d: Overall depth of the column section in mm (float)
       col_f_t: Thickness of flange in mm (float)
       col_w_t: Thickness of flange in mm (float)
       axial_force: Factored axial force in kN (float)

    Returns:
        Force in flange in kN (float)
    """
    axial_force_w = ((col_d - 2(col_f_t)) * col_w_t * axial_force) / total_area
    return round(axial_force_w)                                           # kN
########################################################################################################################
# Thickness of flange splice plate [Reference: N. Subramanian (Page 428), M.L. Gambhir (Page 10.84)]
def thk_flange_plate(col_d, col_f_t, axial_force, moment_load,col_b,col_fy, bolt_dia, dia_hole, edge_dist, pitch):
    """
    Args:
        col_d: Overall depth of the col section in mm (float)
        col_f_t: Thickness of flange in mm (float)
        axial_force: Factored axial force in kN (float)
        moment_load: Factored bending moment in kN-m (float)
        col_b: Width of column flange in mm (float)
        col_fy: Characteristic yield stress in N/mm^2 (float)
    Returns:
    """
    edge_dist = 1.5* dia_hole
    pitch = 2.5 * bolt_dia
    pitch_req = (col_b - (2 * edge_dist))/pitch                   # number of pitch dist required along flange width
    n = pitch_req + 1                                              # number of bolts along flange width

    gamma_m0 = 1.10                                               # Partial safety factor against yield stress and buckling = 1.10 (float)
    f_fcp = flange_force(col_d, col_f_t, axial_force, moment_load)
    flangeplatethickness = f_fcp / ((col_b - n * dia_hole) * (col_fy / (gamma_m0 * 1000)))
...
def flange_capacity(beam_f_t,beam_b,dia_hole,beam_fy):
    ...
    Returns:
        Calculates flange capacity (kN) (float)(
    """
    gamma_m0 = 1.10 # Partial safety factor against yield stress and buckling = 1.10 (float)
...
def web_min_b(col_d):
    """
    Args:
        col_d: Overall depth of supported column (float) in mm
    Returns: Minimum width of web splice plate (float)
    """
    minwebb = round((0.5 * col_d), 2)
    return minwebh

# Maximum width of web splice plate [Reference: Previous DDCL] assumed gap clearance is 5mm based on reasoning
def web_max_b(col_d, col_f_t, col_r1):
    """
    Args:
        col_d: Overall depth of supported column (float) in mm
        col_f_t: Thickness of flange in mm (float)
        col_r1: Root radius of the column section in mm (float)
    Returns: Maximum width of web splice plate in mm (float)
    """
    maxwebwidth = round((col_d - 2 * col_f_t - 2 * col_r1 - 2 * 5), 2)
    return maxwebwidth
...
def web_block_shear_axial( end_dist, n_r, col_t_w, dia_hole, col_d, col_f_t, n_c, fy, fu):
    """
    Args:
        edge_dist: Edge/end distance in mm (Assumption: edge distance = end distance) (float)
        col_t_wp: Thickness of web splice plate in mm (float)
        col_t_w: Thickness of web in mm (float)
        n_c: number of bolt columns(int)
        n_r: number of bolt rows(int)
        col_f_t: Thickness of flange in mm (float)
    ...
    pitch = 2.5 * dia_hole
    gauge_dist = 2.5 * dia_hole
    Avg = 2 * (end_dist + (n_r - 1) * pitch) * col_t_w
    Avn = 2*(end_dist + (n_r - 1) * pitch - (n_r - 0.5) * dia_hole) * col_t_w
    Atg = (col_d - col_f_t - ((n_c - 1) * gauge_dist)) * col_t_w
    Atn = (col_d - col_f_t - ((n_c - 1) * gauge_dist) - (n_c - 0.5) * dia_hole) * col_t_w
...
#  members AISC - overview and design example: Design example 3.1] [link: https://www.aisc.org/globalassets/nsba/design-resources/bolted-field-splices-for-steel-bridge-flexural-members_march2017.pdf]
def web_block_shear(edge_dist, end_dist, col_t_wp, dia_hole, n_r, n_c, fy, fu):
    """
    Args:
        gauge_dist = 2.5 * dia_hole
    Returns: Block shear capacity of web splice plate in kN
    ...
    pitch = 2.5 * dia_hole
    gauge_dist = 2.5 * dia_hole
    Avg = (edge_dist + (n_c - 1) * gauge_dist) * col_t_wp
    Avn = (edge_dist + (n_c - 1) * gauge_dist - (n_c -0.5) * dia_hole)*col_t_wp
    Atg = ((n_r-1)* pitch + end_dist) * col_t_wp
    Atn = ((n_r-1)*pitch + (n_r-1)*dia_hole + end_dist) * col_t_wp
...
#  members AISC - overview and design example: Design example 3.1] [link: https://www.aisc.org/globalassets/nsba/design-resources/bolted-field-splices-for-steel-bridge-flexural-members_march2017.pdf]
def flange_block_shear(end_dist, col_b, col_f_t, gauge_dist,n_c,  pitch, dia_hole, n_r,fy, fu):
    """
    Args:
        edge_dist: Edge/end distance in mm (Assumption: edge distance = end distance) (float)
        col_f_t: Thickness of flange in mm (float)
        col_f_tp: Thickness of flange splice plate in mm (float)
        dia_hole: Diameter of bolt hole in mm
        col_b: Width of flange in mm (float)
    ...
    gamma_m0 = 1.10
    gamma_m1 = 1.25
    Avg = 2 * (end_dist + (n_r - 1) * pitch) * col_f_t
    Avn = 2 * (end_dist + (n_r - 1) * pitch - (n_r - 0.5) * dia_hole) * col_f_t
    Atg = (col_b - (n_c - 1) * gauge_dist) * col_f_t
    Atn = (col_b - ((n_c - 1) * gauge_dist) - (n_c - 1) * dia_hole) * col_f_t
...
#  members AISC - overview and design example: Design example 3.1] [link: https://www.aisc.org/globalassets/nsba/design-resources/bolted-field-splices-for-steel-bridge-flexural-members_march2017.pdf]
def outerflange_plate_block_shear(end_dist, col_f_tp, gauge_dist, n_c,  pitch, dia_hole, n_r,fy, fu):

    gamma_m0 = 1.10
    gamma_m1 = 1.25
    Avg = 2 * (end_dist + (n_r - 1) * pitch) * col_f_tp
    Avn = 2 * (end_dist + (n_r - 1) * pitch - (n_r - 0.5) * dia_hole) * col_f_tp
    Atg = ((n_c - 1) * gauge_dist) * col_f_tp
    Atn = (((n_c - 1) * gauge_dist) - (n_c - 1) * dia_hole) * col_f_tp
...
def innerflange_plate_block_shear(edge_dist, end_dist, col_f_itp, pitch, n_r, n_c, gauge_dist, dia_hole, flange_width, fy, fu):
#Flange splice plate block shear for inner flange splice plate
#pattern 1
    gamma_m0 = 1.10
    gamma_m1 = 1.25
    Avg = (end_dist + (n_r - 1) * pitch)  * col_f_itp
    Avn = (end_dist + (n_r - 1)*pitch - (n_r- 0.5)*dia_hole) * col_f_itp
    Atg = (edge_dist + (n_c - 1)*gauge_dist) * col_f_itp
    Atn = (edge_dist + (n_c - 1)*gauge_dist - (n_c - 0.5) * dia_hole) * col_f_itp
...
def shear_yielding(A_v, beam_fy):
    ...
    """
    gamma_m0 = 1.10
    V_p = (A_v * beam_fy) / (math.sqrt(3) * gamma_m0 * 1000) # kN
    return round(V_p, 2)
...
def coverplateboltedconnection(uiObj):
    ...
    col_fu = float(uiObj["Member"]["fu (MPa)"])
    beam_fy = float(uiObj["Member"]["fy (MPa)"])

    axial_force = str(uiObj["Load"]["AxialForce"])
    moment_load = float(uiObj["Load"]["Moment (kNm)"])
    shear_force = float(uiObj["Load"]["ShearForce (kN)"])

    if shear_force == '':
        shear_force = 0
    else:
        shear_force = float(uiObj["Load"]["ShearForce"])
    ...
    col_f_tp = float(uiObj["FlangePlate"]["Thickness (mm)"])
    col_w_tp = float(uiObj["FlangePlate"]["Width (mm)"])
    ...
    if flange_plate_h == '':
        ...
        flange_plate_h = float(flange_plate_h)
    ...
    if web_plate_h == '':
        ...
        web_plate_h = round(int(web_plate_h), 2)
    ...
    col_w_t= float(dictcolumndata["tw"])
    col_f_t = float(dictcolumndata["T"])
    col_d = float(dictcolumndata["D"])

    col_r1 = float(dictcolumndata["R1"])
    col_b = float(dictcolumndata["B"])
    ...
    if web_plate_t < (col_t_w / 2):  ## Web plate thickness = web plate 1 thickness = (thickness of web / 2)
